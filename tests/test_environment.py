...
def test_step_reward_in_range(env):
    # v4.3: efficiency bonuses allow rewards up to ~1.25 (hard cap 1.30)
    env.reset(task_id=0)
    obs = env.step(SupplyChainAction(tool="get_inventory", args={}))
    assert 0.0 <= obs.reward <= 1.30
...
def test_easy_task_full_reward(env):
    # v4.3: 1-step solve gives ~1.14 with efficiency bonus
    env.reset(task_id=0)
...
def test_reward_never_exceeds_cap(env):
    # v4.3: efficiency bonuses allow rewards up to ~1.25 (hard cap 1.30)
    for tid in [0, 1, 2, 5, 10]:
        env.reset(task_id=tid)
        for _ in range(25):
            obs = env.step(SupplyChainAction(tool="get_inventory", args={}))
            assert obs.reward <= 1.30
            if obs.done:
                break
